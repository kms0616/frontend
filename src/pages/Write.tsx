...
const EFFECT_OPTIONS = [
  { id: "cooling", effectTypeId: 0, label: "냉각력", activeColor: "border-[#00B8ED] bg-[#E5F7FE]", iconColor: "bg-[#00B8ED]", textColor: "text-[#00B8ED]", icon: snowIcon },
  { id: "mental", effectTypeId: 1, label: "정신력", activeColor: "border-[#A27DDB] bg-[#F3EDFD]", iconColor: "bg-[#A27DDB]", textColor: "text-[#A27DDB]", icon: brainIcon },
  { id: "stamina", effectTypeId: 2, label: "체력", activeColor: "border-[#00C772] bg-[#E4FAED]", iconColor: "bg-[#00C772]", textColor: "text-[#00C772]", icon: powerIconn },
  { id: "wealth", effectTypeId: 3, label: "자본력", activeColor: "border-[#FFA300] bg-[#FFF5DE]", iconColor: "bg-[#FFA300]", textColor: "text-[#FFA300]", icon: capitalIcon },
  { id: "endurance", effectTypeId: 4, label: "인내력", activeColor: "border-[#FC504C] bg-[#FEE]", iconColor: "bg-[#FC504C]", textColor: "text-[#FC504C]", icon: patienceIcon },
];
...
export default function SurvivalCreatePage({ onOpenPreview }: SurvivalCreatePageProps) {
  ...
  const [effects, setEffects] = useState<Record<string, { active: boolean; level: number }>>({
  ...
  const handleOpenPreview = () => {
    ...
    const primary = selected.reduce((max, e) => (e.level > max.level ? e : max), selected[0]);
  ...
  return (
    <main className="mx-auto min-h-screen w-[393px] overflow-y-auto rounded-[48px] bg-[#FBFBFB] font-['Pretendard']">
        <header className='flex items-center w-full px-[20px] py-[8px] mt-[54px] mb-[24px]'>
            ...
            <h1 className="absolute left-1/2 -translate-x-1/2 text-[18px] font-[600]">생존법 작성</h1>
        </header>

      <section className="relative px-[20px] pb-[40px]">
        <div className="mb-[16px]">
          <label className="block text-[18px] font-[600] text-[#222] leading-[24px] mb-[8px]">생존법 이름</label>
          <input
            value={name}
        ...
        <div className="mb-[16px]">
          <label className="block text-[18px] font-[600] text-[#222] leading-[24px] mb-[8px]">설명</label>
          <textarea
            value={description}
        ...
        <div className="mb-[24px]">
          <label className="block text-[18px] font-[600] text-[#222] leading-[24px] mb-[8px]">추천 상황</label>
          <input
            value={situation}
        ...
        {/* 효과 */}
        <div className="mb-[24px]">
          <label className="block text-[18px] font-[600] text-[#222] leading-[24px] mb-[8px]">효과 선택</label>
          <p className="text-[14px] text-[#909090] mb-[16px]">1~3가지를 선택 후 레벨을 조절하세요</p>
            ...
            {EFFECT_OPTIONS.map((effect) => {
              ...
              return (
                <div key={effect.id} className="flex flex-col gap-[8px]">
                  ...
                  >
                    <div className={`w-[40px] h-[40px] shrink-0 rounded-[20px] mr-[12px] flex items-center justify-center overflow-hidden ${effect.iconColor}`}>
                       {effect.icon ? (
                        <img src={effect.icon} alt={effect.label} className="w-[24px] h-[24px] object-contain" />
                       ) : null}
                    </div>
                    <span className="font-[600] text-[16px] text-black">{effect.label}</span>

                    {isActive && (
                      <div className={`ml-auto w-[24px] h-[24px] shrink-0 rounded-[20px] flex items-center justify-center ${effect.iconColor}`}>
                        <svg width="13" height="10" viewBox="0 0 13 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M0.75 5.25L4.25 8.75L12.25 0.75" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                        </svg>
                        </div>
                    )}
                  </button>
                  ...
                  {isActive && (
                    <div className={`flex items-center justify-between w-full h-[56px] rounded-[16px] px-[16px] py-[12px] ${effect.activeColor.split(' ')[1]}`}>
                        ...
                        {[1, 2, 3, 4, 5].map((level) => (
                          <button
                            ...
                            className={`w-[40px] h-[40px] rounded-[20px] flex items-center justify-center transition-all ${
                              level <= currentLevel ? effect.iconColor : "bg-white/50"
                            }`}
                          >
                            {effect.icon ? (
                              <img src={effect.icon} alt={`${level}레벨`} className="w-[24px] h-[24px] object-contain" />
                            ) : null}
                          </button>
                        ))}
                      </div>
                      <span className={`font-[600] text-[16px] ${effect.textColor}`}>
                        {currentLevel}
                      </span>
                    </div>
                  )}
          ...
          <div className="flex gap-[8px]">
            {[1, 2, 3, 4, 5].map((star) => {
              ...
              return (
                ...
                >
                  <svg width="36" height="36" viewBox="-2 -2 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
