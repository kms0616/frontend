...
import { EFFECT_OPTIONS } from "../constants/effects";
  ...
  const handleOpenPreview = () => {
    ...
    const selected = Object.entries(effects)
      ...
      .map(([id, v], idx) => {
        ...
        return { effectTypeId: opt.effectTypeId, level: v.level, displayOrder: idx + 1 };
      });
                ...
                <div key={effect.id} className="flex flex-col gap-[8px]">
                  ...
                  {isActive && (
                    <div className={`flex items-center justify-between w-full h-[56px] rounded-[16px] px-[16px] py-[12px] ${effect.activeColor.split(" ")[1]}`}>
                      ...
                      <span className={`font-[600] text-[16px] ${effect.textColor}`}>{currentLevel}</span>
                    </div>
                  )}
                </div>
              );
